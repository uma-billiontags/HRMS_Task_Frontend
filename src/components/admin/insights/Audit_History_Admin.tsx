// Audit_History_Admin.tsx
// Admin-only — GET /api/activity/?task_id=&action=&employee= — sitewide
// ActivityLog feed (capped at 500 rows server-side), most recent first.
...
const ACTION_OPTIONS = [
    "created", "assigned", "started", "paused", "resumed", "submitted",
    "review_started", "approved", "rework_requested", "hold", "released_hold",
    "cancelled", "archived", "correction_requested", "correction_decided",
...
const ACTION_META: Record<string, { color: string; bg: string }> = {
    ...
    archived: { color: "var(--text-muted)", bg: "var(--bg-input)" },
};
...
export default function Audit_History_Admin() {
    ...
    const fetchActivity = useCallback(() => {
        ...
        fetch(`${BASE_URL}/api/tasks/activity/?${params.toString()}`, { headers: { ...authHeaders() } })
            .then((r) => r.json())
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((r) =>
            [r.task_id, r.task_name, r.actor_name, r.action_label].some((f) => f?.toLowerCase().includes(q))
        );
    }, [rows, search]);

    const last24h = rows.filter((r) => dayjs().diff(dayjs(r.created_at), "hour") < 24).length;
    ...
    return (
        <div>
            <div style={{
                ...
                <div>
                    ...
                    <p style={{
                        ...
                        Every recorded action, sitewide
                    </p>
                </div>
            </div>
            ...
            <div className="db-stat-grid" style={{ gridTemplateColumns: "repeat(2, 1fr)" }}>
                <StatCard
                    label="Total Events (last 500)"
                    value={rows.length}
                    changeLabel="Most recent activity feed"
                    changeType="neutral"
                />
                <StatCard
                    label="In Last 24h"
                    value={last24h}
                    changeLabel="Recorded in the past day"
                    changeType="up"
                />
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
                <Input
                    placeholder="Search by task, actor, or action…"
                ...
                <Input
                    ...
                    allowClear
                    style={{
                        width: 160, height: 35, padding: "0 14px",
                        background: "var(--bg-input)", border: "1px solid var(--accent-light)",
                        borderRadius: 9, color: "var(--text-primary)", fontSize: 13, outline: "none",
                    }}
                />
